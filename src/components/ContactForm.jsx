...
export default function ContactForm() {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center px-4 mb-20">
      <div className="bg-white border border-gray-300 px-4 sm:px-6 md:px-10 w-full max-w-[1414px] h-auto py-10 flex items-center justify-center">
        <form
          ref={formRef}
          onSubmit={handleSubmit}
          className="w-full max-w-[1211px] mx-auto grid grid-cols-1 md:grid-cols-2 gap-y-6 md:gap-x-20 font-cormorant text-gray-900"
        >
          {[{ label: "Your Full Name*", name: "full_name", type: "text" },
            ...
            <div key={i} className="flex flex-col space-y-1 w-full">
              <label className="text-[24px] md:text-[36px]">{field.label}</label>
              <input
                type={field.type}
                name={field.name}
          ...
          <div className="md:col-span-2 flex flex-col space-y-1 w-full">
            <label className="text-[24px] md:text-[36px]">Tell me more about the project</label>
            <textarea
              name="message"
